...
# Attach the IAC policy to the TAG IRSA role (the same role used for
# passwordless RDS access), so TAG can read/write S3, SQS, and query Athena.
resource "aws_iam_role_policy" "access_graph_iac" {
  name   = "identity-activity-center"
  role   = aws_iam_role.access_graph_rds.id
  policy = data.aws_iam_policy_document.identity_activity_center_policy.json
}
